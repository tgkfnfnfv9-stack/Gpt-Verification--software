"""Archived H1/H4 preliminary screen. Audit/reproduction only; do not use for formal OANDA validation."""
...
def load_pair(symbol):
    b=load_side(f'data/{symbol}_H1_bid.csv'); a=load_side(f'data/{symbol}_H1_ask.csv'); out=[]
...
def aggregate(src,minutes):
    if minutes==60:return src
    g={}; expected=minutes//60
    for x in src:
        minute=(x.dt.hour*60+x.dt.minute)//minutes*minutes
    ...
    for key,z in sorted(g.items()):
        ...
        if len(z)!=expected or any(z[i].dt!=key+timedelta(minutes=60*i) for i in range(expected)):continue
...
def main():
    files=sorted(Path('data').glob('*_H1_bid.csv')); symbols=[p.name.split('_H1_')[0] for p in files if Path(str(p).replace('_bid.csv','_ask.csv')).exists()]
    results=[]; errors=[]
    for sym in symbols:
        try:
            base=load_pair(sym)
            for tf,m in [('H1',60),('H4',240)]:results.append(cell(sym,tf,aggregate(base,m)))
    ...
    report={'strategy_id':'STRAT-PA-002','spec':'frozen-no-retuning','data_source':'Dukascopy JForex H1 BID+ASK via pinned dukascopy-go v0.2.0 on GitHub Actions','source_period':['2019-08-28','2026-08-28'],'evaluated_until_exclusive':'2025-08-28','final_holdout_2025_08_28_to_2026_08_28':'MASKED_NOT_EVALUATED','timeframes':['H1','H4'],'matched_controls':'same instrument/timeframe/year/UTC4h; nearest prior-only ATR/trend/spread deciles; ±60 days; 5 requested/3 minimum; reuse cap 3','results':results,'ranked':ranked,'errors':errors}
    (OUT/'PA002_cross_market_H1H4_summary.json').write_text(json.dumps(report,ensure_ascii=False,indent=2),encoding='utf-8')
    ...
    (OUT/'PA002_cross_market_H1H4_viewer.json').write_text(json.dumps(viewer,ensure_ascii=False),encoding='utf-8')
